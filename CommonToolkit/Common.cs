...
namespace CommonToolKit
{
    ...
    public class Common
    {
        ...
        /// <summary>
        /// 给字符串添加单引号（如果字符串以英文逗号分隔，则此方法会给每个以英文逗号分隔的字符片段加单引号）
        /// </summary>
        /// <param name="str">需要添加单引号的字符串</param>
        /// <returns></returns>
        public static string StrAddSingleQuote(string str)
        {
            string[] strs = str.Split(new string[] { "," }, StringSplitOptions.RemoveEmptyEntries);
            for (int i = 0; i < strs.Length; i++)
            {
                strs[i] = string.Format("'{0}'", SQLFilter.FilterSQL(strs[i]));
            }
            string newStr = string.Empty;
            foreach (string item in strs)
            {
                newStr += newStr != string.Empty ? "," + item : item;
            }
            return newStr;
        }
